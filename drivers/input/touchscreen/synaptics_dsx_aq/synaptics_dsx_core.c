...
bool asus_rmi4_gesture_check(void);
...
#if 1
void synaptic_usb_detection(bool plugin)
{
	
	int retval = 0;
	char f12_2d_ctrl54[7] = {0};
	if (g_syna_touch_init_status == 1){
		if (plugin){
			/*AC plug in*/
			charge_mode =1;
			printk("[synaptic] charger mode =1 ! \n");
			retval = synaptics_rmi4_reg_read(asus_rmi4_data,0x0021,f12_2d_ctrl54,1);
			if (retval < 0)
				printk("[synaptic] charger mode fail !\n");
			f12_2d_ctrl54[0] = 0x00;
			retval = synaptics_rmi4_reg_write(asus_rmi4_data,0x0021,f12_2d_ctrl54,1);
			if (retval < 0)
				printk("[synaptic] charger mode =1 fail !\n");
		}else{
			/*no AC */
			charge_mode =0;
			printk("[synaptic] charger mode =0 ! \n");
			retval = synaptics_rmi4_reg_read(asus_rmi4_data,0x0021,f12_2d_ctrl54,1);
			if (retval < 0)
				printk("[synaptic] charger mode =0 fail !\n");
			f12_2d_ctrl54[0] = 0x01;
			retval = synaptics_rmi4_reg_write(asus_rmi4_data,0x0021,f12_2d_ctrl54,1);
			if (retval < 0)
				printk("[synaptic] charger mode =0 fail !\n");
		}
...
EXPORT_SYMBOL(synaptic_usb_detection);
#endif
...
static ssize_t asus_boe_enable_glove(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,0x0018,f12_2d_ctrl15,1);
	if (retval < 0)
		return -EINVAL;
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,0x001C,f12_2d_ctrl23,5);
	if (retval < 0)
		return -EINVAL;

	f12_2d_ctrl23[0] |= 0x20;
	retval = synaptics_rmi4_reg_write(rmi4_data,0x001C,f12_2d_ctrl23,5);
	if (retval < 0)
		return -EINVAL;
	//set f12_2d_ctrl23->0x05 --
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,0x0316,&tempvalue,1);
	if (retval < 0)
		return -EINVAL;
...
{	
	if (g_syna_touch_init_status == 1){
		if(enable)
		{
			...
			pr_err("[Touch] : proximity trigger disable touch !\n");
		} 
		else
		{
			SYNAPTICS_TOUCH_DISABLE = false;
			pr_err("[Touch] : proximity trigger enable touch !\n");
		}
	}
}
...
static void synaptics_check_gesture(struct synaptics_rmi4_data *rmi4_data,int gesture_type,int gesture_alphabet)
{
	...
	switch(gesture_type)
	{
		...
		case GESTURE_OTHERS:
			...
			{
				printk("[Touch](E)gesture_alphabet =%x  gesture_mode = %x \n",gesture_alphabet,gesture_mode);
			...
			{
				printk("[Touch](C)gesture_alphabet =%x  gesture_mode = %x \n",gesture_alphabet,gesture_mode);
			...
			{
				printk("[Touch](W)gesture_alphabet =%x  gesture_mode = %x \n",gesture_alphabet,gesture_mode);
			...
			{
			printk("[Touch](S)gesture_alphabet =%x  gesture_mode = %x \n",gesture_alphabet,gesture_mode);
			...
			{
			printk("[Touch](Z)gesture_alphabet =%x  gesture_mode = %x \n",gesture_alphabet,gesture_mode);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture && ((gesture_mode & 0x40) || (dclick_mode == 1) || (swipe_mode ==1))) {
		...
		pr_err("[synaptic][touch]Ps_status= %d \n",Ps_status);
		if(!Ps_status)
			synaptics_check_gesture(rmi4_data,gesture_type,gesture_alphabet);
		else
			pr_err("[synaptic][touch]  skip to report gesture !\n ");
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		if (rmi4_data->fingers_on_2d == false) {
			...
			if (hall_sensor_detect_s)
				msleep(20);
			if ((HALLsensor_gpio_value() > 0) || (key_already_down_s == true))
			{
				input_report_key(rmi4_data->input_dev,
					f1a->button_map[button],
					status);
				key_already_down_s = false;
				if (status)
					key_already_down_s = true;
				printk("[synaptic][touch] send keycode = %d, status = %d\n", f1a->button_map[button], status);
			}
			else
			{
				hall_sensor_detect_s = true;
				printk("[synaptic][touch] disable VK due to hall sensor CLOSE\n");
			}
			...
			if (before_2d_status[button] == 1) {
				...
				if ((HALLsensor_gpio_value() > 0) || (key_already_down_s == true))
				{
					...
					if (status)
						key_already_down_s = true;
					printk("[synaptic][touch] send keycode2 = %d, status = %d\n", f1a->button_map[button], status);
				}
				else
				{
					hall_sensor_detect_s = true;
...
/*****************************************************************************
*  Name: syna_creat_proc_interface
...
*****************************************************************************/
    ...
    static int syna_touch_proc_read(struct seq_file *buf, void *v)
    {
    ...
    static int syna_touch_proc_open(struct inode *inode, struct  file *file)
    {
        return single_open(file, syna_touch_proc_read, NULL);
    }
    
    static void create_syna_touch_proc_file(void)
    {
        static const struct file_operations proc_fops = {
            .owner = THIS_MODULE,
            .open =  syna_touch_proc_open,
            .read = seq_read,
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	queue_delayed_work(exp_data.workqueue,
			&exp_data.work,
			0);
	rmi4_data->suspend_resume_wq = create_singlethread_workqueue("synaptic_resume_wq");
	if (!rmi4_data->suspend_resume_wq) {
			printk("[synaptic][TOUCH_ERR] %s: create resume workqueue failed\n", __func__);
	}
...
#endif
	if (rmi4_data->suspend_resume_wq) {
			destroy_workqueue(rmi4_data->suspend_resume_wq);
		}
...
*/
	static void synaptics_resume_work(struct work_struct *work)
	{
		...
		retval = synaptics_rmi4_reset_device(asus_rmi4_data, false);
		if (retval < 0) {
			dev_err(asus_rmi4_data->pdev->dev.parent,
					"%s: Failed to issue reset command\n",
					__func__);
	...
	exit:
		...
		if((glove_mode == 0) && (cover_mode == 0))
			asus_boe_disable_glove(asus_rmi4_data);
		else
			asus_boe_enable_glove(asus_rmi4_data);
		if(charge_mode == 1)
			synaptic_usb_detection(true);
		else
			synaptic_usb_detection(false);	
...
int asus_rmi4_resume(void)
{
	...
	queue_work(asus_rmi4_data->suspend_resume_wq, &asus_rmi4_data->resume_work);
	msleep(100);
/*	if (asus_rmi4_data->enable_wakeup_gesture && ((gesture_mode & 0x40) || (dclick_mode == 1) || (swipe_mode ==1))) {
		printk("[Touch] : resuem from gesture mode \n");
		disable_irq_wake(asus_rmi4_data->irq);
...
bool asus_rmi4_gesture_check(void)
{
	return (asus_rmi4_data->enable_wakeup_gesture && ((gesture_mode & 0x40) || (dclick_mode == 1) || (swipe_mode ==1)));
}
EXPORT_SYMBOL(asus_rmi4_gesture_check);
...
static int __init synaptics_rmi4_init(void)
{
	...
	printk("[SYNAP][TOUCH] USE IN-CELL !\n");
        printk(KERN_ERR"[JK] Syna init start  \n");
	...
	if (retval)
        ...
        printk(KERN_ERR"[JK] Syna init end  \n");
